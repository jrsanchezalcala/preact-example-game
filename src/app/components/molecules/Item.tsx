import {
  ...
  const getPriceText = (item:Item,currency : string ) => {
    let minimun = item?.currencyData[currency]?.minimumStake ? item?.currencyData[currency]?.minimumStake : null; 
    return minimun ? `${item.currencyData[currency].minimumStake} ${currency} min Stake` : `Not available in ${currency}`
  }
